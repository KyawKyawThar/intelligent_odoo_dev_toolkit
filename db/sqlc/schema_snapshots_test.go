...
func createTestSnapshot(t *testing.T, envID uuid.UUID, modelCount, fieldCount int32) SchemaSnapshot {
	...
	snap, err := testStore.CreateSchemaSnapshot(context.Background(), CreateSchemaSnapshotParams{
		EnvID:       envID,
		Models:      json.RawMessage(`{"res.partner":{"fields":["name","email"]}}`),
		AclRules:    json.RawMessage(`[{"model":"res.partner","group":"base.group_user","read":true}]`),
		RecordRules: json.RawMessage(`[]`),
...
func TestCreateSchemaSnapshot_AllFields(t *testing.T) {
	...
	snap, err := testStore.CreateSchemaSnapshot(context.Background(), CreateSchemaSnapshotParams{
		EnvID:       env.ID,
		Models:      json.RawMessage(`{"res.partner":{},"sale.order":{}}`),
		AclRules:    json.RawMessage(`[{"model":"sale.order","group":"sales_team.group_sale_salesman","read":true,"write":true}]`),
		RecordRules: json.RawMessage(`[{"model":"sale.order","domain":"[('user_id','=',user.id)]"}]`),
		ModelCount:  &mc,
		FieldCount:  &fc,
		DiffRef:     &diffRef,
	})
...
func TestListSchemaSnapshots_DoesNotExposeModels(t *testing.T) {
	// ListSchemaSnapshots returns a lightweight row (no models/acl_rules/record_rules)
	...
	// ListSchemaSnapshotsRow has no Models/AclRules/RecordRules fields
